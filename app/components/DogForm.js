...
const DogForm = ({ onDogAdded }) => {
    const [newDog, setNewDog] = useState({ name: '', breed: '', age: 0, weight: 0, description: '' });
            ...
            uploadTask.on('state_changed',
                ...
                async () => {
                    ...
                    try {
                        ...
                        // Reset the form
                        setNewDog({ name: '', breed: '', age: 0, weight: 0, description: '' });
        ...
        <div className="bg-white shadow container border border-red-500 items-center justify-center w-4/6 mx-auto my-8 flex flex-col px-10 py-5 gap-4 rounded-lg">
            <form onSubmit={addDog}>
                ...
                <div>
                    ...
                    <input min="1" step="1" className='mt-1 block w-full md:w-1/4 border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500' type="number" name="weight" onChange={(e) => setNewDog({ ...newDog, weight: e.target.value ? parseInt(e.target.value) : undefined })} value={newDog.weight || ''} placeholder="Weight" />
                </div>
                {/* Image upload */}
                ...
                <div className='w-full md:w-1/4 mx-auto'>
                    ...
                    <textarea id="description" name="description" className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500" placeholder="Enter description here..." onChange={(e) => setNewDog({ ...newDog, description: e.target.value })} value={newDog.description}></textarea>
                </div>
                {/* Submit button */}
                <button type="submit" className="mt-2 w-full md:w-auto bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2 px-4 rounded shadow-lg hover:shadow-indigo-500/50 transition duration-150 ease-in-out">Submit</button>
            </form>
        </div>
    );
};
